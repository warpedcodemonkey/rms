...
public interface VetPermissionRepository extends JpaRepository<VetPermission, Long> {
    ...
    Optional<VetPermission> findActivePermissionByVetAndAccountWithTime(@Param("vetId") Long vetId, @Param("accountId") Long accountId, @Param("now") LocalDateTime now);

    @Query("SELECT vp FROM VetPermission vp WHERE vp.veterinarian.id = :vetId AND vp.account.id = :accountId AND vp.isActive = true AND (vp.expiresAt IS NULL OR vp.expiresAt > CURRENT_TIMESTAMP)")
    Optional<VetPermission> findActivePermissionByVetAndAccount(@Param("vetId") Long vetId, @Param("accountId") Long accountId);
